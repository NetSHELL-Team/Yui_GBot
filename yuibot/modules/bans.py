...
from yuibot.modules.helper_funcs.extraction import extract_user_and_text
from yuibot.modules.helper_funcs.string_handling import extract_time
from yuibot.modules.log_channel import gloggable, loggable
...
def kick(update: Update, context: CallbackContext) -> str:
    ...
    if res:
        ...
        bot.sendMessage(
            chat.id,
            f"Sayōnara 😏😒  {mention_html(member.user.id, html.escape(member.user.first_name))}.",
            parse_mode=ParseMode.HTML,
        )
        log = (
            f"<b>{html.escape(chat.title)}:</b>\n"
